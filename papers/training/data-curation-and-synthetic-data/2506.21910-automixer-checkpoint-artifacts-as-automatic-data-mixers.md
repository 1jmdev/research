# AutoMixer: Checkpoint Artifacts as Automatic Data Mixers

**arXiv:** [2506.21910](https://arxiv.org/abs/2506.21910) · [PDF](https://arxiv.org/pdf/2506.21910) · [HTML](https://arxiv.org/html/2506.21910) · **First submitted:** 2026-02-08  
**Category:** [Data curation, mixtures & synthetic data](README.md) — **rank #113 of 247** by impact score  

> **TL;DR** — This work observes that checkpoint models exhibit emerging capabilities at different points in the training trajectory and identifies these artifact models based on their respective capabilities on the benchmarks and leverage them as data mixers by using their aggregated first-order influence approximation over source data.

## At a glance

| Field | Value |
| --- | --- |
| Authors | Ernie Chang, Yang Li, Patrick Huber, Vish Vogeti, David Kant, Yangyang Shi, Vikas Chandra |
| arXiv categories | cs.CL |
| Venue | Accepted at ACL 2025 |
| arXiv comments | Accepted at ACL 2025 |
| Citations (Semantic Scholar, 2026-09-25) | 7 (1 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Training requirement (from abstract) | involves (re)training / fine-tuning |
| Impact score | 3.44 |

## Key results (claimed in abstract)

- We demonstrated on eight reasoning benchmarks that the proposed framework shows significant improvements in the pretraining setting, with performance improvements of up to 1.93%.

## Abstract

In language model training, it is desirable to equip models with capabilities from various tasks. However, it is not clear how to directly obtain the right data mixtures for these capabilities as the relationship between data and tasks is difficult to be modeled. In this work, we observe that checkpoint models exhibit emerging capabilities at different points in the training trajectory. Often, the training process saves checkpoints as artifacts that are under-utilized as a source of in-training data signals. We identify these artifact models based on their respective capabilities on the benchmarks and leverage them as data mixers by using their aggregated first-order influence approximation over source data. We demonstrated on eight reasoning benchmarks that the proposed framework shows significant improvements in the pretraining setting, with performance improvements of up to 1.93%. Overall, this shows the potential of checkpoint models to enhance data quality and optimize data mixtures.

## Reproduce / read

```sh
arxiv read 2506.21910            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2506.21910 "method"
arxiv bibtex 2506.21910
```
