# Data Scout: Targeted Web Crawling for Domain-Specific Pretraining Corpora

**arXiv:** [2609.05766](https://arxiv.org/abs/2609.05766) · [PDF](https://arxiv.org/pdf/2609.05766) · [HTML](https://arxiv.org/html/2609.05766) · **First submitted:** 2026-09-04  
**Category:** [Data curation, mixtures & synthetic data](README.md) — **rank #244 of 247** by impact score  
**Also relevant to:** [Pre-training recipes & efficiency](../pretraining-recipes-and-efficiency/README.md)  

> **TL;DR** — Data Scout is presented: instead of filtering an archive, it directs a targeted crawl, and the probe is the only domain-specific component, which can in principle apply to any domain with such a classifier.

## At a glance

| Field | Value |
| --- | --- |
| Authors | Chirag Garg, Eelaaf Zahid, Farhan Ahmed, Jay Pankaj Gala, Eric Butler, Heiko Ludwig |
| arXiv categories | cs.LG, cs.AI |
| Citations (Semantic Scholar, 2026-09-25) | 0 (0 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Training requirement (from abstract) | involves (re)training / fine-tuning |
| Models used | Llama-3.2-3B |
| Impact score | 0.0 |

## Key results (claimed in abstract)

- This works because relevance has a sharp boundary at the subdomain level: in mathematics, a page is 21x more likely to be relevant than one on a sibling subdomain.
- With the FineMath classifier as the probe, 21.9% of crawled pages are high-quality math content, 70x the 0.31% rate from filtering a comparable web sample, so the crawl wastes far less effort.
- But the payoff is not just efficiency: 63.2% of these pages are missing from CommonCrawl altogether, yet just as useful for training.

## Abstract

The dominant approach to building domain-specific pretraining corpora is to filter large web archives such as CommonCrawl. This works well for popular domains but breaks down for specialized ones, where relevant content is sparse and often beyond the reach of popularity-driven crawlers. We present Data Scout, which inverts this: instead of filtering an archive, it directs a targeted crawl. An LLM expands a root topic into a taxonomy and thousands of search queries; the returned URLs (seeds) are grouped by subdomain and screened with a user-supplied classifier (the probe), admitting each subdomain on the basis of a small sample. This works because relevance has a sharp boundary at the subdomain level: in mathematics, a page is 21x more likely to be relevant than one on a sibling subdomain. With the FineMath classifier as the probe, 21.9% of crawled pages are high-quality math content, 70x the 0.31% rate from filtering a comparable web sample, so the crawl wastes far less effort. But the payoff is not just efficiency: 63.2% of these pages are missing from CommonCrawl altogether, yet just as useful for training. Continued pretraining of Llama-3.2-3B on 1.9B Data Scout tokens matches FineMath corpus on GSM8k. Because the probe is the only domain-specific component, Data Scout can in principle apply to any domain with such a classifier.

## Reproduce / read

```sh
arxiv read 2609.05766            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2609.05766 "method"
arxiv bibtex 2609.05766
```
