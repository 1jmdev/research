...
> **TL;DR** — This work hypothesizes that the two distributions a byte-level model produces, one over the next byte, one over where its patch boundaries fall, can be disentangled and changed almost independently, and argues that the community should move toward a byte-level interface as a shared standard.
