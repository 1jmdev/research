...
> **TL;DR** — A theoretical analysis of the effect of vocabulary size on training dynamics is provided, and it is revealed that in the LV regime, the optimal embedding LR to hidden LR ratio should roughly scale as $\Theta(\sqrt{width})$, surprisingly close to the empirical findings previously reported in the literature, and different from the $\Theta(width)$ ratio predicted by $\mu$P.
