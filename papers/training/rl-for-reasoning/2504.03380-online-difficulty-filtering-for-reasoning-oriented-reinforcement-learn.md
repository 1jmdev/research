# Online Difficulty Filtering for Reasoning Oriented Reinforcement Learning

**arXiv:** [2504.03380](https://arxiv.org/abs/2504.03380) · [PDF](https://arxiv.org/pdf/2504.03380) · [HTML](https://arxiv.org/html/2504.03380) · **First submitted:** 2026-01-25  
**Category:** [Reinforcement learning for LLM reasoning (GRPO, RLVR)](README.md) — **rank #98 of 516** by impact score  

> **TL;DR** — This work presents a formal analysis of online difficulty-aware filtering and establishes its theoretical foundations, showing that expected policy improvement is lower-bounded by the variance of task-level success probabilities, and that selecting tasks of intermediate difficulty maximizes learning efficiency.

## At a glance

| Field | Value |
| --- | --- |
| Authors | Sanghwan Bae, Jiwoo Hong, Min Young Lee, Hanbyul Kim, JeongYeon Nam, Donghyun Kwak |
| arXiv categories | cs.CL, cs.AI |
| Venue | Conference of the European Chapter of the Association for Computational Linguistics (Conf Eur Chapter Assoc Comput Linguistics) |
| arXiv comments | To appear in EACL 2026 (main conference) |
| Citations (Semantic Scholar, 2026-09-25) | 104 (26 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Impact score | 10.88 |

## Key results (claimed in abstract)

- Evaluations across multiple math reasoning benchmarks validate that balanced filtering consistently enhances convergence speed and final performance, achieving up to +12% gains in less than half the training steps of standard GRPO.

## Abstract

Recent advances in reinforcement learning with verifiable rewards (RLVR) show that large language models enhance their reasoning abilities when trained with verifiable signals. However, due to reward sparsity, effectiveness depends heavily on selecting samples of appropriate difficulty. In this work, we present a formal analysis of online difficulty-aware filtering and establish its theoretical foundations. We show that expected policy improvement is lower-bounded by the variance of task-level success probabilities, implying that selecting tasks of intermediate difficulty maximizes learning efficiency. Building on this, we demonstrate that balanced filtering maximizes this lower bound, leading to superior performance and sample efficiency. Evaluations across multiple math reasoning benchmarks validate that balanced filtering consistently enhances convergence speed and final performance, achieving up to +12% gains in less than half the training steps of standard GRPO. By extending our analysis to various reward distributions, we provide a principled foundation for future RLVR curriculum strategies, confirmed through both theoretical analysis and extensive empirical results.

## Reproduce / read

```sh
arxiv read 2504.03380            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2504.03380 "method"
arxiv bibtex 2504.03380
```
