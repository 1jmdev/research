...
**Category:** [Reinforcement learning for LLM reasoning (GRPO, RLVR)](README.md) — **rank #467 of 516** by impact score  

> **TL;DR** — This work proposes energy-based fine-tuning (EBFT), which uses strided block-parallel sampling to generate multiple rollouts from nested prefixes concurrently, batches feature extraction over these rollouts, and uses the resulting embeddings to perform an on-policy policy-gradient update.
