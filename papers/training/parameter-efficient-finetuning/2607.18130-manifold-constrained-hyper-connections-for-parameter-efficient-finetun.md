# Manifold-Constrained Hyper-Connections for Parameter-Efficient Finetuning

**arXiv:** [2607.18130](https://arxiv.org/abs/2607.18130) · [PDF](https://arxiv.org/pdf/2607.18130) · [HTML](https://arxiv.org/html/2607.18130) · **First submitted:** 2026-07-20  
**Category:** [Parameter-efficient fine-tuning (LoRA & friends)](README.md) — **rank #111 of 117** by impact score  
**Also relevant to:** [Novel architectures & architecture analysis](../../models-and-architectures/novel-architectures/README.md) · [MoE architecture & routing](../../mixture-of-experts/architecture-and-routing/README.md)  

> **TL;DR** — Manifold-Constrained Hyper-Connections, a generalisation of residual connections, is investigated as a novel PEFT approach, wrapping frozen OLMo-2 backbones with learned residual routing modules, identifying residual routing as a distinct and promising novel PEFT axis.

## At a glance

| Field | Value |
| --- | --- |
| Authors | Valentijn Oldenburg, Floris de Kam, Bente Zuijdam, Lieve Eberson, Nicky van Zutphen, Stef de Wildt, Ivo Verhoeven |
| arXiv categories | cs.LG |
| arXiv comments | Pre-print |
| Citations (Semantic Scholar, 2026-09-25) | 0 (0 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Training requirement (from abstract) | involves (re)training / fine-tuning |
| Models used | OLMo-2 |
| Impact score | 0.0 |

## Abstract

Most parameter-efficient finetuning (PEFT) methods adapt weights or activations, thus leaving one of the key Transformer components unchanged: residual connections. This paper investigates Manifold-Constrained Hyper-Connections (mHC), a generalisation of residual connections, as a novel PEFT approach, wrapping frozen OLMo-2 backbones with learned residual routing modules. We find that mHC can finetune frozen Transformers, but that its role differs fundamentally from the original pre-training setting: in finetuning, fixing the residual mixing matrix to identity often improves performance. As a standalone PEFT method, mHC does not consistently outperform LoRA. However, at matched trainable parameter budgets, mHC+LoRA combinations improve language-modelling loss and show task-dependent benchmark gains at both 1B and 7B scale. Overall, our results identify residual routing as a distinct and promising novel PEFT axis.

## Reproduce / read

```sh
arxiv read 2607.18130            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2607.18130 "method"
arxiv bibtex 2607.18130
```
