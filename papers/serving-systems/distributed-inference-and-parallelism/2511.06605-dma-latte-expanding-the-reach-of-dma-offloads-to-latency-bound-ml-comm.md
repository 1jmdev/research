# DMA-Latte: Expanding the Reach of DMA Offloads to Latency-bound ML Communication

**arXiv:** [2511.06605](https://arxiv.org/abs/2511.06605) · [PDF](https://arxiv.org/pdf/2511.06605) · [HTML](https://arxiv.org/html/2511.06605) · **First submitted:** 2026-09-14  
**Category:** [Distributed inference & parallelism (TP/PP/SP/CP/EP)](README.md) — **rank #30 of 180** by impact score  
**Also relevant to:** [Serving & inference systems — general](../_general/README.md)  

> **TL;DR** — This work leverages hitherto untapped features available in the state-of-the-art AMD Instinct GPUs that render DMA communication offloads competitive even in latency-bound regions, and demonstrates the efficacy of these features both at the operator level (ML communication collectives such as all-gather and all-to-all), and at the end-to-end workload level (LLM inference).

## At a glance

| Field | Value |
| --- | --- |
| Authors | Suchita Pati, Shaizeen Aga, Mahzabeen Islam, Ryan Quach, Saleel Kudchadker, Mohamed Assem Ibrahim |
| arXiv categories | cs.DC, cs.AR |
| Venue | MICRO 2026 |
| arXiv comments | Accepted to appear in the 59th IEEE/ACM International Symposium on Microarchitecture (MICRO 2026) |
| Citations (Semantic Scholar, 2026-09-25) | 2 (0 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Impact score | 3.12 |

## Key results (claimed in abstract)

- At the operator level, our optimizations provide up to 4.5$\times$ speedups (3.2$\times$ geomean in the latency-bound region) over baseline DMA offload, narrowing the performance gap while delivering additional power savings (3-10%) for ML collectives compared to state-of-the-art GPU core-based communication library, RCCL.
- At the workload level, we demonstrate acceleration for LLM inference: up to 1.65$\times$ lower latency and up to 1.9$\times$ higher throughput over the state-of-the-art vLLM inference framework.

## Abstract

Offloading communication to existing direct memory access (DMA) engines, available on most state-of-the-art commercial GPUs, has emerged as a practical and low-cost solution to efficiently overlap computation and communication in machine learning (ML). However, the reach of DMA offloads has so far been limited to bandwidth-bound scenarios only (10s of MB to GB transfer sizes). In this work, we break this barrier and extend DMA communication offloads to latency-bound regions (KB to low MB). Specifically, we leverage hitherto untapped features available in the state-of-the-art AMD Instinct$^{\mathrm{TM}}$ GPUs that render DMA communication offloads competitive even in latency-bound regions. We demonstrate the efficacy of these features both at the operator level (ML communication collectives such as all-gather and all-to-all), and at the end-to-end workload level (LLM inference). At the operator level, our optimizations provide up to 4.5$\times$ speedups (3.2$\times$ geomean in the latency-bound region) over baseline DMA offload, narrowing the performance gap while delivering additional power savings (3-10%) for ML collectives compared to state-of-the-art GPU core-based communication library, RCCL. At the workload level, we demonstrate acceleration for LLM inference: up to 1.65$\times$ lower latency and up to 1.9$\times$ higher throughput over the state-of-the-art vLLM inference framework. We conclude with a discussion of AMD Instinct GPU runtime innovations that stand to expose these features.

## Reproduce / read

```sh
arxiv read 2511.06605            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2511.06605 "method"
arxiv bibtex 2511.06605
```
