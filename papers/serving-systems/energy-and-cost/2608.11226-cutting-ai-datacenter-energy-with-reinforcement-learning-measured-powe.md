...
**arXiv:** [2608.11226](https://arxiv.org/abs/2608.11226) · [PDF](https://arxiv.org/pdf/2608.11226) · [HTML](https://arxiv.org/html/2608.11226) · **First submitted:** 2026-07-27  
**Category:** [Energy, carbon & cost of LLM inference/training](README.md) — **rank #62 of 69** by impact score  

> **TL;DR** — A GRPO training with half-second power telemetry at 7B, 14B, and 72B scales, and a PPO meta-controller that adapts the workload's own generation parameters to measured power, which results in the best mean throughput and energy per token among constrained controllers.
...
| arXiv categories | cs.AI, cs.SY, eess.SY |
...
- Against the full 500-step 7B trace, the controller cuts power-limit violations by 89.8% while increasing token output by 18.1% and energy efficiency by 26.2% (tokens per MWh).
- An actuator-authority sweep shows the same parameters applied as generation concurrency retain 17-22% power authority, isolating an occupancy-versus-volume principle; a controller rebuilt on that actuator controls a live 72B rollout-generation workload across three replications: 35.7% more output than a static safe baseline at 2.27 +/- 1.08% budget violations, 87.2% fewer violations than uncontrolled operation, and the best mean throughput and energy per token among constrained controllers, with an adaptive threshold rule matching it in one of three operating conditions.
- Under realistic measurement windows the original 72B transients fall from 23.6% at half-second resolution to 1.6% at 30 s and zero at 5 min; a composed 16-GPU fleet shows zero violations at 30 s and longer, with peak demand at 50-56% of nameplate.
...
```sh
arxiv read 2608.11226            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2608.11226 "method"
arxiv bibtex 2608.11226
```
