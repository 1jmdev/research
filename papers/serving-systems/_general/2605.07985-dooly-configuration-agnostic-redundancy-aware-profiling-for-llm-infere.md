...
**Category:** [Serving & inference systems — general](README.md) — **rank #92 of 117** by impact score  

> **TL;DR** — Dooly performs a single inference pass, labels each input dimension with its origin via taint propagation, and selectively profiles only operations absent from its latency database, which builds latency regression models based on the database, which becomes a drop-in backend for existing simulators.
