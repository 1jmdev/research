...
**Category:** [Sparse attention (trainable & training-free)](README.md) — **rank #70 of 181** by impact score  

> **TL;DR** — This work proposes ZETA, leveraging \textbf{Z}-Order Curves for \textbf{E}fficient \textbf{T}op-$k$ \textbf{A}ttention, to enable parallel querying of past tokens for entire sequences, and proposes reducing the dimensionality of keys and queries in contrast to values and using Z-order curves to map low-dimensional keys and queries into low-dimensional space, which permits parallel sorting.
