...
**arXiv:** [2607.21535](https://arxiv.org/abs/2607.21535) · [PDF](https://arxiv.org/pdf/2607.21535) · [HTML](https://arxiv.org/html/2607.21535) · **First submitted:** 2026-07-23  
**Category:** [Multi-token prediction (MTP)](README.md) — **rank #22 of 25** by impact score  
**Also relevant to:** [Speculative decoding (draft–verify)](../speculative-decoding/README.md)  

> **TL;DR** — Across three architecture families at 1M context on a single GPU in SGLang, windowing cuts the per-decode-step cost over the shipping native MTP draft by +28% to +44%, an input-invariant margin that widens with context.
...
| arXiv comments | 25 pages, 2 figures, 11 tables |
...
- It bounds the draft's KV working set to a constant, dropping ~99% of KV entries at 1M.
- Across three architecture families (Qwen GDN-MoE 35B/122B and a Mamba2-hybrid NoPE 120B) at 1M context on a single GPU in SGLang, windowing cuts the per-decode-step cost over the shipping native MTP draft by +28% to +44%, an input-invariant margin that widens with context.
- Finally, the unread draft KV -- 7.7-11% of total KV at 1M -- is reclaimed via a compact ring buffer at no acceptance or quality cost.
...
```sh
arxiv read 2607.21535            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2607.21535 "method"
arxiv bibtex 2607.21535
```
