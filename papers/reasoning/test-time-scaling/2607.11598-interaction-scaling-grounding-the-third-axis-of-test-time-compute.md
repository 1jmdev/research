...
**arXiv:** [2607.11598](https://arxiv.org/abs/2607.11598) · [PDF](https://arxiv.org/pdf/2607.11598) · [HTML](https://arxiv.org/html/2607.11598) · **First submitted:** 2026-07-13  
**Category:** [Test-time scaling & inference-time compute](README.md) — **rank #220 of 227** by impact score  

> **TL;DR** — It is argued that a single variable governs this third axis, grounding, and that it must hold on both sides of the loop, which means interaction scaling is real and distinct from reasoning and sampling, but only visible when both the feedback and the metric are grounded.
...
- On hard coding tasks at a fixed token budget, reasoning-only and best-of-N sampling both plateau (the latter even when an oracle picks the best sample), while every interaction strategy keeps improving; our proposer-reviewer harness reaches a perfect 100% pass rate with no run-to-run variance, and the gain holds across three model families.
- A tool that measures the real layout instead shows the loop removing 40-74% of defects across four modalities; and that same VLM, used as the reviewer, makes slide layouts worse where the measuring tool repairs them.
...
```sh
arxiv read 2607.11598            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2607.11598 "method"
arxiv bibtex 2607.11598
```
