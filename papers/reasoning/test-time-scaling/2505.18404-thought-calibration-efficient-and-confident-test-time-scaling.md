# Thought calibration: Efficient and confident test-time scaling

**arXiv:** [2505.18404](https://arxiv.org/abs/2505.18404) · [PDF](https://arxiv.org/pdf/2505.18404) · [HTML](https://arxiv.org/html/2505.18404) · **First submitted:** 2025-05-23  
**Category:** [Test-time scaling & inference-time compute](README.md) — **rank #97 of 227** by impact score  

> **TL;DR** — This work proposes thought calibration to decide dynamically when thinking can be terminated, which preserves model performance with up to a 60% reduction in thinking tokens on in-distribution data, and up to 20% in out-of-distribution data.

## At a glance

| Field | Value |
| --- | --- |
| Authors | Menghua Wu, Cai Zhou, Stephen Bates, Tommi Jaakkola |
| arXiv categories | cs.LG, cs.AI |
| Venue | Conference on Empirical Methods in Natural Language Processing (Empir Method Nat Lang Process) |
| Citations (Semantic Scholar, 2026-09-25) | 23 (2 influential) |
| Hugging Face upvotes | 0 |
| Code | — |
| Impact score | 4.77 |

## Key results (claimed in abstract)

- Based on three reasoning language models and four datasets, thought calibration preserves model performance with up to a 60% reduction in thinking tokens on in-distribution data, and up to 20% in out-of-distribution data.

## Abstract

Reasoning large language models achieve impressive test-time scaling by thinking for longer, but this performance gain comes at significant compute cost. Directly limiting test-time budget hurts overall performance, but not all problems are equally difficult. We propose thought calibration to decide dynamically when thinking can be terminated. To calibrate our decision rule, we view a language model's growing body of thoughts as a nested sequence of reasoning trees, where the goal is to identify the point at which novel reasoning plateaus. We realize this framework through lightweight probes that operate on top of the language model's hidden representations, which are informative of both the reasoning structure and overall consistency of response. Based on three reasoning language models and four datasets, thought calibration preserves model performance with up to a 60% reduction in thinking tokens on in-distribution data, and up to 20% in out-of-distribution data.

## Reproduce / read

```sh
arxiv read 2505.18404            # full paper as Markdown (github.com/1jmdev/arxiv)
arxiv section 2505.18404 "method"
arxiv bibtex 2505.18404
```
